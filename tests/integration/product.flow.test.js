...
const PRODUCT_CHEAP_BOOK = { title: "Cheap Book", price: 5, category: CATEGORY_BOOKS, description: "A cheap book" };
const PRODUCT_EXPENSIVE_BOOK = { title: "Expensive Book", price: 50, category: CATEGORY_BOOKS, description: "An expensive book" };
const PRODUCT_GADGET = { title: "Cool Gadget", price: 30, category: CATEGORY_GADGETS, description: "A cool gadget" };
...
describe("Product flow: admin adds categories & products, user filters list", () => {
  ...
  it("admin adds two categories", async () => {
    const booksRes = await adminAgent.agent.post("/api/category/").send({ name: CATEGORY_BOOKS });
    expect(booksRes.status).toBe(200);
    expect(booksRes.body.category.name).toBe(CATEGORY_BOOKS);

    const gadgetsRes = await adminAgent.agent.post("/api/category/").send({ name: CATEGORY_GADGETS });
    expect(gadgetsRes.status).toBe(200);
    expect(gadgetsRes.body.category.name).toBe(CATEGORY_GADGETS);
  });

  it("admin adds three products across both categories", async () => {
    const res1 = await adminAgent.agent.post("/api/product/").send(PRODUCT_CHEAP_BOOK);
    expect(res1.status).toBe(200);
    expect(res1.body.product.title).toBe(PRODUCT_CHEAP_BOOK.title);

    const res2 = await adminAgent.agent.post("/api/product/").send(PRODUCT_EXPENSIVE_BOOK);
    expect(res2.status).toBe(200);

    const res3 = await adminAgent.agent.post("/api/product/").send(PRODUCT_GADGET);
  ...
  it("filters by category returns only books", async () => {
    const res = await userAgent.agent.get(`/api/product/?category=${CATEGORY_BOOKS}`);

    expect(res.status).toBe(200);
    expect(res.body.products.length).toBe(2);
    expect(res.body.products.every((p) => { return p.categoryId !== undefined; })).toBe(true);
  });
  ...
  it("filters by title search is case-insensitive", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body.products.length).toBe(2);
    expect(res.body.products.every((p) => { return p.title.toLowerCase().includes("book"); })).toBe(true);
  });
  ...
  it("combining category and max price filters narrows results", async () => {
    const res = await userAgent.agent.get(`/api/product/?category=${CATEGORY_BOOKS}&price=10`);
  ...
  it("blocks adding duplicate product title", async () => {
    const res = await adminAgent.agent.post("/api/product/").send(PRODUCT_CHEAP_BOOK);
